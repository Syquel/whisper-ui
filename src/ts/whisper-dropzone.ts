...
function initSendWhisperFilesDropZone(listeners: FilesDroppedListener[]) {
    const dropZoneElement = document.getElementById("whisper-dropzone");
    const dropZoneInputElement = document.getElementById("whisper-dropzone-input");
    initDropZone(dropZoneElement, dropZoneInputElement, listeners);
}

function initRecipientDropZone(listeners: FilesDroppedListener[]) {
    const dropZoneElement = document.getElementById("add-recipient-dropzone");
    const dropZoneInputElement = document.getElementById("add-recipient-dropzone-input");
    initDropZone(dropZoneElement, dropZoneInputElement, listeners);
}

function initReceivedWhisperFilesDropZone(listeners: FilesDroppedListener[]) {
    const dropZoneElement = document.getElementById("whisper-received-dropzone");
    const dropZoneInputElement = document.getElementById("whisper-received-dropzone-input");
    initDropZone(dropZoneElement, dropZoneInputElement, listeners);
}
...
function initDropZone(dropZoneElement: HTMLElement, dropZoneInputElement: HTMLInputElement, listeners: FilesDroppedListener[]) {
    ...
    [Event.DragLeave, Event.DragEnd].forEach(
        (type) => {
            dropZoneElement.addEventListener(
                type, () => {
